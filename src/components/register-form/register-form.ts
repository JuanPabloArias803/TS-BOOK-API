...
export function RegisterFormLogic(){
    ...
    const $userName=document.querySelector("#register-form-userName") as HTMLInputElement;
    const $userLastName=document.querySelector("#register-form-userLastName") as HTMLInputElement;
    const $userEmail=document.querySelector("#register-form-userEmail") as HTMLInputElement;
    const $userPassword=document.querySelector("#register-form-userPassword") as HTMLInputElement;
    const api=new ApiInteraction;
    $registerForm.addEventListener("submit",async (event)=>{
        event.preventDefault();
        if(!$userName.value||!$userLastName.value||!$userEmail.value||!$userPassword.value){
            alert("Registro inválido");
            return;
        }
        await api.createUser({name:$userName.value,lastName:$userLastName.value,email:$userEmail.value,password:$userPassword.value});
        NavigateTo('/login');
    });
}