...
export async function EditBookFormLogic(bookID: string) {
  ...
  const $bookPublicationDate = document.querySelector(
    '#edit-form-publicationDate'
  ) as HTMLInputElement;
  const api = new ApiInteraction();
  const currentBook = await api.consultOneBook(bookID); //get current book info
  ...
  const setDate = date.toISOString().substring(0, 10).split('-'); //fix local date problems and separate YYYY-MM-DD
