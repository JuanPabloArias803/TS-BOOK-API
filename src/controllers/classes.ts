...
export class ApiInteraction {
  ...
  async authUser(user: IUserLogin): Promise<void> {
    ...
    try {
      ...
      sessionStorage.setItem(
        'UT',
        CryptoJS.AES.encrypt(responseData.data.token, cryptoKey).toString()
      ); //Save encrypted Role in sessionStorage
      sessionStorage.setItem(
        'UR',
        CryptoJS.AES.encrypt(responseData.data.role, cryptoKey).toString()
      ); //Save encrypted Token in sessionStorage
    } catch (error) {
      alert(error);
    }
  }
    ...
    const options = {
      ...
      headers: {
        Authorization: `Bearer ${CryptoJS.AES.decrypt(
          token,
          cryptoKey
        ).toString(CryptoJS.enc.Utf8)}`, //decrypt token
      },
    };
  ...
  async consultOneBook(bookID: string): Promise<IBook> {
    ...
    try {
      const response: Response = await fetch(
        `${this.domain}/api/v1/books/${bookID}`,
        options
      );
      if (!response.ok) {
        throw `Error inesperado en el sistema. (${response.status}: ${response.statusText})`;
